"""
特徵訓練腳本（M4：影分身 table_train.py）
使用 common/* 共同模組，讀取 data.csv 表格資料集，
依 label 型別自動選擇分類（softmax）或回歸（linear）任務，產出 TFLite 模型。
特徵類型＝MediaPipe Hand/Pose landmark → data.csv（欄位經 featureSchema.js 契約）；
訓練管線與 table 共用 table_dataset.py（MLP、label 分層/回歸）。
"""
...
def plot_table_curves(history, output_path, project_name, epochs, task):
    ...
    try:
        ...
        ax1.set_title(f'{project_name} - Feature Training Curves', fontsize=14, fontweight='bold')
        ax1.legend(loc='best')
...
<div class="container">
  ...
  <div class="footer">Generated by Cocoya AI Training (Feature)</div>
</div>
</body>
</html>'''
