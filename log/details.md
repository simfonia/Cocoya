...
- **技術**: 在 Python 注入 `MusicEngine` 類別，使用專則表達式 `([A-GR][#S]?)([0-8])?([WHQEST])(\.)?(_T)?` 解析旋律。
...
### 2. 前端架構重構與註解保留
- **策略**：將 `main.js` 中的通用邏輯（XML 過濾、產生器覆寫、搜尋注入）移至 `utils.js` 的 `CocoyaUtils` 工具集。
...
- **停用狀態連動**：由於孤兒積木檢查是在 Events.disable() 下執行，Minimap 無會透過事件自動更新顏色。修正為在狀態變更完成後手動觸發 
efreshMinimap 進行強制同步。

## 關鍵技術實作紀錄 (2026-04-26 更新)

### 1. 實體自動備份與還原系統 (Physical Auto-Backup)
- **問題**：學校環境常見的電腦還原系統（C 碟重開機即清空）導致儲存在 `localStorage` 的學生進度經常遺失。
- **方案**：改用後端檔案系統儲存實體備份檔（`.xml.bak`）。
- **路徑策略**：
    - 已存檔專案：備份於專案路徑旁，命名為 `.[檔名].xml.bak` (隱藏檔)。這確保了隨身碟或 D 碟上的進度能被隨專案保存。
    - 未存檔 (Untitled)：備份於擴充功能安裝路徑下的 `temp_scripts/untitled_backup.xml`。
- **分叉封存機制 (Archive Forking)**：為了解決「拒絕恢復後隨即更動」會覆蓋舊備份的問題，當使用者在恢復彈窗點擊「取消」時，後端會將備份檔改名為 `.bak.old_[timestamp]`。這保留了學生珍貴的心血，即便事後反悔也能手動救回。

### 2. Tauri v2 核心指令補完 (Rust)
- **序列埠偵測**：實作 `get_serial_ports` 調用 `serialport` Rust 庫，提供比原先 PowerShell 腳本更穩定的裝置列舉能力。
- **MCU 部署**：實作 `deploy_mcu` 指令，對接 `deploy_mcu.py` 輔助腳本。
- **啟動偵測**：在 `get_manifest` (啟動必經之路) 加入備份檢查。若發現未命名備份，Rust 會透過 Tauri `Emitter` 推送事件至前端，達成一啟動即詢問恢復的效果。
