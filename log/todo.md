...
## [已完成] 里程碑 v2.1: MediaPipe 手勢整合 (2026-02-17)
- [x] **MediaPipe 手勢偵測 (ai_hand)**
    - [x] 模型初始化與特徵點獲取。
    - [x] 座標正規化與 21 個關節點索引。
    - [x] 邏輯判斷 (如：手指伸直偵測)。
    - [x] 在影像上標註手部特徵點與連結線。

## [已完成] 里程碑 v2.2: 臉部網格與 AR 技術 (2026-02-17)
- [x] **MediaPipe 臉部網格 (ai_face)**
    - [x] 初始化、偵測、網格繪製、範圍標註點位。
- [x] **進階 AR 標註 (cv_draw)**
    - [x] 實作帶有 Alpha 透明度的影像覆蓋積木。
    - [x] 全面修正座標型別 (Float to Int) 轉換。
- [x] **系統穩定化**
    - [x] 實作跨端非同步 Prompt/Confirm 橋樑。
    - [x] 解決變數重新命名無效之問題。

## [待辦] 里程碑 v2.3: 姿勢偵測
- [ ] **MediaPipe 姿勢偵測 (ai_pose)**
    - [ ] 33 個特徵點獲取與骨架繪製。
    - [ ] 肢體動作判斷 (如：舉手、跳躍)。
