...
- [x] **Webview 深度重構**：CSS 抽離、CSP 強化、NaN 屬性攔截、代碼模組化。
- [x] **πCar 功能全面移植**：對齊 #piBlockly 規格，包含運動、感測、音樂與 NeoPixel 系統。
- [x] **MCU 基礎腳位控制**：實作數位輸出/入、類比讀入、PWM 輸出與 I2C 掃描。
- [x] **[New] 腳位選取系統優化**：改為分組建議下拉選單，對齊 Seeed Studio 官方規範。
- [x] **[New] 積木說明文件系統 (Help System)**：實作圖形化對照表與 Webview-Host 開啟橋樑。
