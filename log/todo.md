...
- **（本次追加，已修）`_handleExamplesSaveDialog` 與 `_handleCloseDialog` fallback 併入 token 化**（2026-09-01）：覆蓋範例確認框改走 `_showConfirmDialog`（i18n label 訊息），關閉 fallback 一併改；tauri.js 內 `@tauri-apps/plugin-dialog` 殘留 = 0。
- **下一切片：Stage 6 收尾**——歷史計畫文件標記 superseded（DatasetManagerOptimization/ProgressAndGuardrails/DATASET_MANAGER_PLAN.md）、system_spec.html Dataset Manager 章節核對 → Stage 6 Gate（§9.3）簽核 → Stage 7 總驗證。
