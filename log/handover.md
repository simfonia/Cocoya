...
- **進度**：
  - **[UI/UX]** 為「深度修復」選單加入詳細的 Hover 說明（Tooltip），支援 中英雙語。
...
## 2026-05-01 Tauri 全面穩定化與終端機實作 (里程碑 v7.0 完成)
- **進度**：
  - **[Tauri 除錯]** 徹底解決本地資源載入報錯 (highlight.js, favicon, py_definition_zone) 與平台自動遷移 (CircuitPython -> MicroPython)。
  - **[多視窗隔離]** 重構 AppState 為 HashMap 結構。實現按 `window.label()` 隔離檔案路徑與 Python 進程，解決多視窗狀態污染問題。
  - [Tauri] 實現 Webview 內建終端機面板。支援自動捲動開關 (預設 ON)、1000 行效能保護、深色捲軸與即時日誌轉發 (Unbuffered)。
    - [更新機制] 對齊 #wavecode 實作檢查更新動畫、版本號 Tooltip 顯示與啟動自動檢查。
    - [硬體強化] 
      - 序列埠：對齊 VSIX 顯示格式，支援 PID 偵測與智慧型號辨識。
      - 深度修復：重構為遞迴檔案清理 (Wipe)，加入 DTR/RTS 控制與連線衝突自動釋放機制。
    - [UI/UX] 優化工具列佈局、補齊 i18n Tooltip、修正停止按鈕功能與視覺回饋。
  - [目前狀態]：Cocoya Tauri 獨立版功能已完全對齊並在部分體驗上超越 VSIX 版。

## 2026-05-03 UI/UX 響應式優化與邊界防護
- **核心進度**：
  - **[響應式佈局]**：針對 Tauri 與 Webview 模式實作工具列響應式設計。
    - 實作 `.toolbar-group` 自動換行，防止在窄視窗下功能按鈕溢出至代碼區域。
    - 實作「動態路徑標籤」：隨寬度縮減最大寬度 (180px -> 120px)，並在極窄模式下自動隱藏以確保核心按鈕可用。
  - **[邊界限制]**：在 `tauri.conf.json` 強制加入 `minWidth: 960` 與 `minHeight: 600`，從物理層面解決版面崩潰問題。
- **目前狀態**：介面穩定性與適配性大幅提升。
